<div class="card">
    <div class="card-header">
        <h3 class="card-title"><i class="fas fa-money-bill mr-2"></i>Kasa İşlemleri - <?php echo sanitize($cash_register['name']); ?></h3>
        <div class="card-tools">
            <a href="<?php echo url('cash/addTransaction/' . $cash_register['id']); ?>" class="btn btn-sm btn-primary">
                <i class="fas fa-plus mr-2"></i>Yeni İşlem
            </a>
        </div>
    </div>
    <div class="card-body">
        <form method="GET" action="<?php echo url('cash/transactions/' . $cash_register['id']); ?>" class="mb-3">
            <div class="row">
                <div class="col-md-3">
            ...
            <tbody>
                <?php foreach ($transactions as $transaction): ?>
                    <tr>
                        ...
                        <td>
                            <a href="<?php echo url('cash/editTransaction/' . $transaction['id']); ?>" class="btn btn-sm btn-primary">
...
function deleteTransaction(id) {
    if (confirm('İşlemi silmek istediğinizden emin misiniz?')) {
        $.post('<?php echo url('cash/deleteTransaction'); ?>', {id: id}, function(response) {
            if (response.success) {
                location.reload();
            } else {
...
function controlTransaction(id, status) {
    if (confirm('İşlemi ' + (status == 'controlled' ? 'kontrol etmek' : 'reddetmek') + ' istediğinizden emin misiniz?')) {
        $.post('<?php echo url('cash/controlTransaction'); ?>', {id: id, status: status}, function(response) {
            if (response.success) {
                location.reload();
            } else {
